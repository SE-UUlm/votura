...
export const VotingSuccessView = (): JSX.Element => {
  ...
  return (
    <Center mih="100vh">
      <Container w={600}>
        <Stack gap="lg">
          <Alert icon={<IconCheck size={18} />} color="green" variant="filled">
            {t('voteSuccessfullyCast', 'We have received your vote for {{electionName}}.', {
              electionName: state?.electionName
                ? state.electionName
                : t('standardElection', 'the Election'),
            })}
          </Alert>

          <Button fullWidth onClick={(): void | Promise<void> => navigate('/votingHome')}>
